...
class GmailToolBase(BaseTool):
    ...
    #             logger.info(f"search_criteria:{search_criteria}")
    ...
    #         return thread_messages
    #     except Exception as e:
    #         logger.info(f"While getting thread message:{str(e)}")
...
class GetUnreadEmailsTool(GmailToolBase):
    ...
    def _run(self, limit: Optional[int] = None) -> List[Tuple[str, str, str, str, Dict]]:
        ...
        try:
            ...
            logger.info(f"DEBUG: Exception in GetUnreadEmailsTool: {e}")
            import traceback
...
class SaveDraftTool(BaseTool):
    ...
    def _run(self, email_id:str,subject: str, body: str, recipient: str, thread_info: Optional[Dict[str, Any]] = None,draft_saved:Optional[bool]=False) -> str:
        try:
